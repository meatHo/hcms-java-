...
class CarTime{
    ...
    void calarrivaltime(Car car,int distance){//도착시간계산완료 윤년, 매달마다 30일 31일 그거 해 완료
        ...
        if(arrivaltime.getmonth()==2){
            if(arrivaltime.getyear()%4==0){//윤년
                ...
                if(arrivaltime.getyear()%400!=0&&arrivaltime.getyear()%100==0){
...
class CarLoc{
    ...
    public static double calposition(final Time departtime,Time time){
        double currentmin=0;
        double departmin=0;
        ...
        if(current.getyear()>departtime.getyear()){
            ...
            if(departtime.getmonth()==4||departtime.getmonth()==6||departtime.getmonth()==9||departtime.getmonth()==11){
                ...
                if(departtime.getyear()%4==0){
                    if(departtime.getyear()%400!=0&&departtime.getyear()%100==0){
    ...
    void setposition(Car car,CarTime cartime,Time time){
